...
const EditProject = () => {
  ...
  useEffect(() => {
    if (existingProject && projectTypes.length > 0) {
      ...
      // pre-select contractor types from saved data
  ...
  const handleCreateDashboard = async () => {
    ...
    try {
      ...
      setShowDashboardModal(false);
      setDashboardDate("");
      setTimeout(() => navigate(`/dashboard/timeline/${H_ID}`, { state: { projectStartDate: dashboardDate } }), 1000);
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to create dashboard");
      setIsCreatingDashboard(false);
    }
      ...
      <div className="p-6 bg-white shadow rounded-lg mt-8">
        ...
        <Form {...form}>
          ...
          >
                    ...
                    {existingMandatoryDocs.map((doc) => (
                      <li key={doc.ID}
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {doc.FILE_SIZE && (
                            <span className="text-gray-400 shrink-0 text-xs">{fmtBytes(doc.FILE_SIZE)}</span>
                          )}
                        </div>
                        <a
                  ...
                  {newMandatoryFiles.map((file, idx) => (
                    <li key={idx}
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-gray-400 shrink-0 text-xs">{fmtBytes(file.size)}</span>
                      </div>
            ...
            {/* ══ Action Buttons ══════════════════════════════════════════ */}
            <div className="col-span-3 flex flex-wrap justify-end gap-2 mt-4 pt-4 border-t">
              {/* Update */}
              <Button type="submit" disabled={updateMutation.isPending}>
                <Save size={16} className="mr-2" />
                {updateMutation.isPending ? "Updating..." : "Update Project"}
              ...
              <div className="relative group">
                ...
                >
                  {processAlreadyCreated ? (
                    <><CheckCircle size={16} className="mr-2 text-green-500" />Process Created</>
                  ) : (
              ...
              <div className="relative group">
                ...
                >
                  {dashboardAlreadyCreated ? (
                    <><CheckCircle size={16} className="mr-2 text-green-500" />Dashboard Created</>
                  ) : "Create Dashboard"}
                </Button>
                {dashboardAlreadyCreated && (
                  <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 whitespace-nowrap
        ...
        <div className="overflow-x-auto mt-8">
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {editableLines.length > 0 ? editableLines.map((line, index) => (
                <tr key={line.ID || index} className="hover:bg-gray-50">
                  <td className="px-3 py-2 border w-[5%]">{line.PROCESS_ID}</td>
