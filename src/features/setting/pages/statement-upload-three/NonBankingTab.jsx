...
export default function NonBankingTab({ projectOptions, contractorOptions, projectOpts, contractorOpts, mutations, sortBy = "txnDate" }) {
  ...
  const handleNbSubmit = () => {
    if (!nbForm.txnDate || !nbForm.amount) { toast.error("Date and Amount are required."); return; }
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${url}/api/statement/staging/all?${params.toString()}`);
      if (!downloadCsv(res.data?.data || [], "statement_nonbanking.csv")) toast.error("No rows to download.");
    } catch {
