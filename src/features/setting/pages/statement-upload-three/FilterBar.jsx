// src/features/setting/pages/statement-upload-three/FilterBar.jsx
...
const FilterBar = React.memo(function FilterBar({
  ...
  useEffect(() => { setDraft(initialFilters); }, [initialFilters]);
    ...
    <div className="bg-white border rounded-2xl shadow-sm px-5 py-4 mb-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2">
        <div>
          ...
          <DateInput value={draft.dateFrom} onChange={(v) => update("dateFrom", v)} />
        </div>
        <div>
          <label className="text-[10px] text-gray-400 block mb-0.5">Date To</label>
          <DateInput value={draft.dateTo} onChange={(v) => update("dateTo", v)} />
        </div>
        {showStatus && (
          <div>
            <label className="text-[10px] text-gray-400 block mb-0.5">Status</label>
            <Select value={draft.status || "ALL"} onValueChange={(v) => update("status", v === "ALL" ? "" : v)}>
        ...
        <div>
          ...
          <Input placeholder="Invoice no." value={draft.invoiceNo} onChange={(e) => update("invoiceNo", e.target.value)} className="h-8 text-xs" />
        </div>
        <div>
          <label className="text-[10px] text-gray-400 block mb-0.5">Amount Min</label>
          <Input type="number" step="1" placeholder="Min" value={draft.amountMin} onChange={(e) => update("amountMin", e.target.value)} className="h-8 text-xs" />
        </div>
        <div>
          <label className="text-[10px] text-gray-400 block mb-0.5">Amount Max</label>
          <Input type="number" step="1" placeholder="Max" value={draft.amountMax} onChange={(e) => update("amountMax", e.target.value)} className="h-8 text-xs" />
        </div>
        <div>
          <label className="text-[10px] text-gray-400 block mb-0.5">Description</label>
          <Input placeholder="Search description" value={draft.description} onChange={(e) => update("description", e.target.value)} className="h-8 text-xs" />
        </div>
        {showCategory && (
          <div>
            <label className="text-[10px] text-gray-400 block mb-0.5">Category</label>
            <Select value={draft.category || "ALL"} onValueChange={(v) => update("category", v === "ALL" ? "" : v)}>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleSearch} disabled={!hasPendingChanges}
            ...
            {hasActive && (
              <Button size="sm" variant="outline" onClick={handleClear} className="h-8 px-2.5 rounded-md text-xs">
                Reset <X size={12} />
              </Button>
            )}
          </div>
