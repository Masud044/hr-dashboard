...
// ── Component ─────────────────────────────────────────────────────────────────
export function CreateCalendarSheet({ isOpen, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: async (formData) => axios.post(`${url}/api/calendar`, formData),
    onSuccess: () => {
    ...
    onError: (err) => {
      toast.error(err?.response?.data?.message || "Failed to create calendar day.");
    },
  });
    ...
    <Sheet open={isOpen} onOpenChange={handleClose}>
      <SheetContent className="!w-screen !h-screen !max-w-none overflow-y-auto flex flex-col gap-0 p-0 rounded-none z-[104]">
        <SheetHeader className="mb-4 px-6 pt-6">
          <SheetTitle className="text-lg font-semibold">Add Calendar Day</SheetTitle>
          <hr />
        </SheetHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="px-6 pb-8">
            ...
            <div className="flex justify-between gap-3 mt-8 pt-4 border-t">
              ...
              <Button type="submit" disabled={mutation.isPending}>
                {mutation.isPending ? "Saving..." : "Save Day"}
              </Button>
            </div>
          </form>
        </Form>
